...
export async function fetchActivites(dossierId: string): Promise<ActiviteRow[]> {
  try {
    ...
    return rows.map((a) => ({
      ...
      hypothese: (a.hypothese ?? "COMMUNE") as ActiviteRow["hypothese"],
...
export async function fetchActivitesCommission(
  ...
  try {
    ...
    return rows.map((a) => ({
      id: a.id,
      libelle: a.libelle,
      hypothese: (a.hypothese ?? "COMMUNE") as ActiviteCommissionRow["hypothese"],
      montantN: Number(a.montantN),
      evolutionN1: Number(a.evolutionN1),
      montantN1: Number(a.montantN1),
      evolutionN2: Number(a.evolutionN2),
...
export async function fetchProductionsImmobilisees(
  ...
  try {
    ...
    return rows.map((p) => ({
      ...
      hypothese: (p.hypothese ?? "COMMUNE") as ProductionImmobiliseeRow["hypothese"],
      date: p.date ?? "",
...
export async function fetchSubventionsExploitation(
  ...
  try {
    ...
    return rows.map((s) => ({
      id: s.id,
      libelle: s.libelle,
      hypothese: (s.hypothese ?? "COMMUNE") as SubventionExploitationRow["hypothese"],
