...
import { fetchSeuilRentabilite, type BreakEvenData } from "@/app/actions/controle/seuil-rentabilite";

// ── Types ─────────────────────────────────────────────────────────────────────

export type SeuilRentabiliteStatus = "idle" | "loading" | "success" | "error";
...
interface SeuilRentabiliteState {
  cache: Record<string, CacheEntry>;
  status: Record<string, SeuilRentabiliteStatus>;
  errors: Record<string, string>;

  getData: (dossierId: string) => BreakEvenData | null;
  getStatus: (dossierId: string) => SeuilRentabiliteStatus;
...
export const useSeuilRentabiliteStore = create<SeuilRentabiliteState>((set, get) => ({
  cache: {},
  status: {},
  errors: {},
  ...
  fetch: async (dossierId, force = false) => {
    ...
    try {
      const data = await fetchSeuilRentabilite(dossierId);
      set((s) => ({
