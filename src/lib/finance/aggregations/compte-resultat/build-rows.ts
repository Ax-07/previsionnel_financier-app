...
import type { DrilldownRows } from "./drilldown";
import { mkVal, pct, makeNodeBuilder, buildChildNodes } from "../helpers/shared-helpers";
import type { CRChildRow } from "./helpers";
...
// ── Agrégats dérivés (display-only ou dépendants de DrilldownRows) ─────────────
...
export function buildCRTree(
  ...
  const ca = fc.ca;
  const node = makeNodeBuilder<CRNode>(ca);
  const childNodes = <TRow extends CRChildRow>(rows: TRow[], key: string) =>
    buildChildNodes<TRow, CRNode>(rows, key, ca, (r) => r.actif !== false);
  ...
  return [
    ...
    ...caGroups.map((g) =>
      node(`ca_group_${g.type}`, g.label, g.subtotal, "normal",
        g.rows.map((r) => ({
          key: `ca_group_${g.type}_child_${r.id}`,
          label: r.libelle,
          values: {
    ...
    ...(nonZero(fc.commissionsTotal)
      ? [node("commissions", "Commissions", fc.commissionsTotal, "normal",
        commissionRows.map((r) => ({
          key: `commission_child_${r.id}`,
          label: r.libelle,
          values: {
            y1: mkVal(r.montantN, pct(r.montantN, ca.y1)),
            y2: mkVal(r.montantN1, pct(r.montantN1, ca.y2)),
    ...
    ...(nonZero(fc.reprises) ? [node("reprises", "Reprises sur provisions", fc.reprises, "normal", childNodes(reprisesRows, "reprises"), true)] : []),
    ...
    node("charges_expl_header", "CHARGES D'EXPLOITATION", fc.totalChargesExpl, "section"),

    ...(nonZero(fc.achatsEffectues)
      ? [node("achats", "Achats effectués de matières / marchandises", fc.achatsEffectues, "normal",
        [...childNodes(achatsRows, "achats")],
        true,
      )]
      : []),

    ...(nonZero(agg.varStockDisplay) ? [node("variation_stock", "Variation de stocks", agg.varStockDisplay, "normal", undefined, true)] : []),
    ...(nonZero(fc.fournitures) ? [node("fournitures", "Fournitures consommables", fc.fournitures, "normal", childNodes(fournituresRows, "fournitures"), true)] : []),
    ...(nonZero(fc.services) ? [node("services", "Services extérieurs", fc.services, "normal", childNodes(servicesRows, "services"), true)] : []),

    node("charges_ext", "Charges externes (Total)", fc.chargesExternes, "subtotal"),


    node("salaires_bruts", "Salaires bruts (Salariés)", fc.chargesPersonnel.salairesBruts, "normal", childNodes(salaireRows, "salaires"), true),
    node("charges_sociales", "Charges sociales (Salariés)", fc.chargesPersonnel.chargesPatronales, "normal", undefined, true),
    node("remunerations_dir", "Rémunération du dirigeant", fc.chargesPersonnel.remuDirigeant, "normal", childNodes(dirigeantRows, "dirigeants"), true),
    node("cotisations_tns", "Cotisations TNS", fc.chargesPersonnel.cotisationsTNSTotal, "normal", childNodes(cotisationsRows, "cotisations_tns"), true),
    node("taxes_salaires", "Taxes assises sur les salaires", fc.chargesPersonnel.taxesSalairesTotal, "normal", childNodes(taxesSalairesRows, "taxes_sal"),
      taxesSalairesRows.length === 0,
    ),
    node("charges_personnel", "Charges de personnel (Total)", fc.chargesPersonnel.total, "subtotal"),

    node("impots_taxes", "Impôts et taxes", fc.impotsTaxes, "normal", childNodes(impotsRows, "impots"), true),
    node("dotations_amort", "Dotations aux amortissements", fc.dotationsAmort, "normal", dotationsParImmoData.length > 0
        ? [
    ...
    ...(nonZero(fc.dotationsProvisions) ? [node("dotations_provisions", "Dotations sur provisions", fc.dotationsProvisions, "normal", childNodes(provisionsRows, "provisions"), true)] : []),
    ...(nonZero(fc.autresChargesGestion) ? [node("autres_charges_gestion", "Autres charges de gestion courante", fc.autresChargesGestion, "normal", childNodes(chargesGestionRows, "charges_gestion"), true)] : []),

    node("total_charges_expl", "Total des charges d'exploitation", fc.totalChargesExpl, "total"),
    ...
    ...(nonZero(fc.produitsFinanciers)
      ? [node("produits_fin", "Produits financiers", fc.produitsFinanciers, "normal", childNodes(prodFinRows, "prod_fin"), true)]
      : []),

    ...(nonZero(fc.chargesFinTotal)
      ? [
        node("charges_fin", "Charges financières (dont intérêts emprunts)", fc.chargesFinTotal, "normal",
          [
            ...(interetsParEmprunt.length > 0
              ? [
                node("charges_fin_interets", "Intérêts et assurances emprunts",
    ...
    ...(nonZero(agg.produitsExcep)
      ? [node("produits_excep", "Produits exceptionnels", agg.produitsExcep, "normal", childNodes(prodExcepRows, "prod_excep"), true)]
      : []),
    ...(nonZero(agg.chargesExcep)
      ? [node("charges_excep", "Charges exceptionnelles", agg.chargesExcep, "normal", childNodes(chargesExcepRows, "charges_excep"), true)]
      : []),
