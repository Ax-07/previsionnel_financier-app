...
import { saveServices } from "@/app/actions/charges";
...
function GroupSummaryServicesExterieurs({ rows, dossierId }: { rows: ChargeExploitationRow[]; dossierId: string }) {
  const hypotheseActive = useHypotheseStore((s) => s.getActive(dossierId));
  const { showN1, showN2 } = useExercicesDisplay(dossierId);
  const activeRows = filterByHypothese(rows, hypotheseActive).filter((r) => r.actif !== false);
  return (
...
export interface TableauServicesExterieursProps {
  dossierId: string;
...
export function TableauServicesExterieurs({
  dossierId,
  initialData,
  dateDebutExerciceN,
  exercices,
}: TableauServicesExterieursProps) {
  ...
  const isDirty = rows.some((r) => r._dirty) || (draft._deletedServiceIds?.length ?? 0) > 0;

  const setRows = useCallback(
    (updater: (prev: ChargeExploitationRow[]) => ChargeExploitationRow[]) => {
      const d = useChargesStore.getState().getDraft(dossierId);
      setServicesRows(dossierId, updater(d.services));
    },
    [dossierId, setServicesRows],
  );
  ...
  const saveAll = useCallback(() => {
    startTransition(async () => {
      try {
        const d = useChargesStore.getState().getDraft(dossierId);
        const result = await saveServices(dossierId, d.services);
        if (result.success) {
          if (result.idMap && Object.keys(result.idMap).length > 0) {
            const idMap = result.idMap;
            setServices(dossierId, (prev) => prev.map((r) => ({ ...r, id: r.id && idMap[r.id] ? idMap[r.id] : r.id })));
          }
  ...
  }, [dossierId, invalidateControleStores, markServicesSaved, setServices]);
  ...
  const renderRow = useCallback(
        ...
        >
          ...
          <Td className="text-center px-1">
            <input
              ...
              onChange={(e) => updateServiceRow(dossierId, idx, { actif: e.target.checked })}
            />
          </Td>
          ...
          <Td>
            <input
              ...
              onChange={(e) => updateServiceRow(dossierId, idx, { libelle: e.target.value })}
            />
          </Td>
          ...
          <Td>
            <select
              ...
              value={row.hypothese}
              onChange={(e) =>
                updateServiceRow(dossierId, idx, { hypothese: e.target.value as ChargeExploitationRow["hypothese"] })
              }
            >
          ...
          <Td className={cn(row.detailCalc?.modeCalc === "POURCENTAGE_CA" ? "bg-muted/20 text-muted-foreground" : "")}>
            {row.detailCalc?.modeCalc === "POURCENTAGE_CA" ? (
              ...
              <input
                ...
                min={0}
                step={0.01}
                onChange={(e) => updateServiceRow(dossierId, idx, { montantN: numVal(e.target.value) })}
              />
            )}
          </Td>
          ...
          {showN1 && (
            <Td>
              <input
                ...
                onChange={(e) => updateServiceRow(dossierId, idx, { evolutionN1: numVal(e.target.value) })}
              />
            </Td>
          )}
          ...
          {showN2 && (
            <Td>
              <input
                ...
                step={0.1}
                disabled={row.detailCalc?.modeCalc === "POURCENTAGE_CA"}
                onChange={(e) => updateServiceRow(dossierId, idx, { evolutionN2: numVal(e.target.value) })}
              />
            </Td>
          )}
          ...
          <Td>
            <input
              ...
              max={100}
              step={1}
              onChange={(e) => updateServiceRow(dossierId, idx, { tauxFixe: numVal(e.target.value) })}
            />
          </Td>
          ...
          <Td>
            <select
              ...
              value={row.frequence}
              onChange={(e) =>
                updateServiceRow(dossierId, idx, { frequence: e.target.value as ChargeExploitationRow["frequence"] })
              }
            >
          ...
          <Td>
            <select
              ...
              onChange={(e) => updateServiceRow(dossierId, idx, { delaiReglement: parseInt(e.target.value, 10) })}
            >
          ...
          <Td>
            <select
              className={cellSelect}
              value={row.tauxTVA}
              onChange={(e) => updateServiceRow(dossierId, idx, { tauxTVA: parseFloat(e.target.value) })}
            >
          ...
          <Td>
            <select
              ...
              value={row.typeTVA}
              onChange={(e) =>
                updateServiceRow(dossierId, idx, { typeTVA: e.target.value as ChargeExploitationRow["typeTVA"] })
              }
            >
          ...
          <Td className="text-center px-1">
            <RowActions
              onDelete={() => removeServiceRow(dossierId, idx)}
              onDuplicate={() => duplicateServiceRow(dossierId, idx)}
              isPending={isPending}
    ...
    [rows, showN1, showN2, isPending, updateServiceRow, dossierId, removeServiceRow, duplicateServiceRow],
  );
  ...
  return (
    <div className="space-y-3">
      <SectionHeader
        ...
        isSaving={isPending}
        onSave={saveAll}
        onAdd={() => addServiceRow(dossierId)}
        onAddGroup={() => addServiceGroup(dossierId)}
      />
      <GroupedDndTable
        dnd={dnd}
        colSpan={2 + (showN1 ? 2 : 0) + (showN2 ? 2 : 0)}
        onAddRowToGroupe={(groupe) => addServiceToGroup(dossierId, groupe)}
        renderRow={renderRow}
        ...
        renderGroupSummaryCells={(groupRows) => (
          <GroupSummaryServicesExterieurs rows={groupRows as LocalChargeExploitationRow[]} dossierId={dossierId} />
        )}
      >
