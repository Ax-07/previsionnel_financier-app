...
const CATEGORIE = "FINANCIER" as const;
...
export function TableauProduitsFinanciers({
  dossierId,
  initialData,
}: {
  dossierId: string;
  ...
  const rows = draft.financiers;
  const isDirty = draft.hasUnsavedFinanciers;
  ...
  const setRowsDnd = useCallback(
    (updater: (prev: AutreProduitDateRow[]) => AutreProduitDateRow[]) => {
      store.setFinanciers(dossierId, updater(rows));
    },
        ...
        >
          ...
          <Td className="w-8 px-2">
            <input
              ...
              onChange={(e) => store.updateFinancier(dossierId, i, { actif: e.target.checked })}
          ...
          <Td>
            <input
              className={cellInput}
              value={row.libelle}
              onChange={(e) => store.updateFinancier(dossierId, i, { libelle: e.target.value })}
              placeholder="Libellé"
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN ?? ""}
              onChange={(e) => store.updateFinancier(dossierId, i, { dateN: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN}
              min={0}
              onChange={(e) => store.updateFinancier(dossierId, i, { montantN: numVal(e.target.value) })}
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN1 ?? ""}
              onChange={(e) => store.updateFinancier(dossierId, i, { dateN1: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN1}
              min={0}
              onChange={(e) => store.updateFinancier(dossierId, i, { montantN1: numVal(e.target.value) })}
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN2 ?? ""}
              onChange={(e) => store.updateFinancier(dossierId, i, { dateN2: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN2}
              min={0}
              onChange={(e) => store.updateFinancier(dossierId, i, { montantN2: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-10 px-1">
            <div className="flex items-center justify-center gap-0.5 opacity-0 group-hover:opacity-100">
              ...
              <Button size="icon" variant="ghost" className="h-6 w-6 text-primary" onClick={() => store.duplicateFinancier(dossierId, i)}>
                <Copy className="h-3 w-3" />
              </Button>
              <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => store.removeFinancier(dossierId, i)}>
  ...
  return (
    <div className="space-y-3">
      <SectionHeader
        ...
        isDirty={isDirty}
        isSaving={isPending}
        onAdd={() => store.addFinancier(dossierId)}
        onSave={handleSave}
