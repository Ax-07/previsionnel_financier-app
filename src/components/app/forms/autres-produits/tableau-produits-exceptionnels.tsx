...
const CATEGORIE = "EXCEPTIONNEL" as const;
...
export function TableauProduitsExceptionnels({
  dossierId,
  initialData,
}: {
  dossierId: string;
  ...
  const rows = draft.exceptionnels;
  const isDirty = draft.hasUnsavedExceptionnels;
  const [isPending, startTransition] = useTransition();
  const invalidateControleStores = useInvalidateControleStores();

  useEffect(() => {
    if (rows.length === 0 && initialData.length > 0) {
      store.setExceptionnels(dossierId, initialData);
    }
  ...
  const handleSave = useCallback(() => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        store.markExceptionnelsSaved(dossierId, fresh);
  ...
  const setRowsDnd = useCallback(
    (updater: (prev: AutreProduitDateRow[]) => AutreProduitDateRow[]) => {
      store.setExceptionnels(dossierId, updater(rows));
    },
  ...
  const addGroupe = useCallback(() => {
    ...
    store.setExceptionnels(dossierId, [
      ...rows,
      {
        ...
        >
          ...
          <Td className="w-8 px-2">
            <input
              ...
              onChange={(e) => store.updateExceptionnel(dossierId, i, { actif: e.target.checked })}
          ...
          <Td>
            <input
              className={cellInput}
              value={row.libelle}
              onChange={(e) => store.updateExceptionnel(dossierId, i, { libelle: e.target.value })}
              placeholder="Libellé"
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN ?? ""}
              onChange={(e) => store.updateExceptionnel(dossierId, i, { dateN: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN}
              min={0}
              onChange={(e) => store.updateExceptionnel(dossierId, i, { montantN: numVal(e.target.value) })}
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN1 ?? ""}
              onChange={(e) => store.updateExceptionnel(dossierId, i, { dateN1: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN1}
              min={0}
              onChange={(e) => store.updateExceptionnel(dossierId, i, { montantN1: numVal(e.target.value) })}
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN2 ?? ""}
              onChange={(e) => store.updateExceptionnel(dossierId, i, { dateN2: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN2}
              min={0}
              onChange={(e) => store.updateExceptionnel(dossierId, i, { montantN2: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-20">
            <select
              ...
              value={row.tauxTVA ?? 0}
              onChange={(e) =>
                store.updateExceptionnel(dossierId, i, {
          ...
          <Td className="w-10 px-1">
            <div className="flex items-center justify-center gap-0.5 opacity-0 group-hover:opacity-100">
              ...
              <Button size="icon" variant="ghost" className="h-6 w-6 text-primary" onClick={() => store.duplicateExceptionnel(dossierId, i)}>
                <Copy className="h-3 w-3" />
              </Button>
              <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => store.removeExceptionnel(dossierId, i)}>
  ...
  return (
    <div className="space-y-3">
      <SectionHeader
        ...
        description="Indemni tés, gains non récurrents…"
        isDirty={isDirty}
        isSaving={isPending}
        onAdd={() => store.addExceptionnel(dossierId)}
        onSave={handleSave}
