...
export function TableauReductionCapital({
  dossierId,
  initialData = [],
}: {
  dossierId: string;
  ...
  const setRows = useCallback(
    (updater: (prev: DiversOperationCapitalRow[]) => DiversOperationCapitalRow[]) =>
      store.setReductionsCapitalRows(
        dossierId,
        updater(store.getDraft(dossierId).reductionsCapital),
      ),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [dossierId],
  );
  ...
  const addRow = () => store.addReductionCapital(dossierId);
  const addGroupe = () => store.addReductionsCapitalGroup(dossierId);
  const addRowToGroupe = (groupe: string) => store.addReductionsCapitalToGroup(dossierId, groupe);
  const updateRow = (id: string, data: Partial<DiversOperationCapitalRow>) => store.updateReductionCapital(dossierId, id, data);
  const removeRow = (id: string) => store.removeReductionCapital(dossierId, id);
  const duplicateRow = (id: string) => store.duplicateReductionCapital(dossierId, id);
