...
import { fetchChargesBilan, saveChargesBilan } from "@/app/actions/autres-charges";
...
const TYPE = "CHARGE_CONSTATEE_AVANCE" as const;
...
export function TableauChargesConstateesAvance({
  dossierId,
  initialData,
}: {
  dossierId: string;
  ...
  const handleSave = useCallback(() => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        store.markCCASaved(dossierId, fresh);
  ...
  const setRowsStore = useCallback(
    (newRows: AutreChargeBilanRow[]) => store.setCCA(dossierId, newRows),
    [dossierId, store],
  );

  const setRowsDnd = useCallback(
    (updater: (prev: AutreChargeBilanRow[]) => AutreChargeBilanRow[]) => setRowsStore(updater(rows)),
    [rows, setRowsStore],
  );
        ...
        >
          ...
          <Td className="w-8 px-2">
            <input
              ...
              onChange={(e) => store.updateCCA(dossierId, i, { actif: e.target.checked })}
          ...
          <Td>
            <input
              className={cellInput}
              value={row.libelle}
              onChange={(e) => store.updateCCA(dossierId, i, { libelle: e.target.value })}
              placeholder="Libellé"
            />
          </Td>
          <Td>
            <select
              ...
              value={row.nature}
              onChange={(e) => store.updateCCA(dossierId, i, { nature: e.target.value })}
            >
              <option value="" className="bg-background text-foreground">—</option>
              {NATURES_CHARGE_BILAN_CCA.map((n) => (
          ...
          <Td className="w-28">
            <input
              ...
              value={row.montantN}
              min={0}
              onChange={(e) => store.updateCCA(dossierId, i, { montantN: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN1}
              min={0}
              onChange={(e) => store.updateCCA(dossierId, i, { montantN1: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN2}
              min={0}
              onChange={(e) => store.updateCCA(dossierId, i, { montantN2: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-10 px-1">
            <div className="flex items-center justify-center gap-0.5 opacity-0 group-hover:opacity-100">
              <Button size="icon" variant="ghost" className="h-6 w-6 text-primary" onClick={() => store.duplicateCCA(dossierId, i)}>
                <Copy className="h-3 w-3" />
              </Button>
              <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => store.removeCCA(dossierId, i)}>
