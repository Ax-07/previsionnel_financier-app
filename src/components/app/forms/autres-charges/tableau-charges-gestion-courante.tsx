...
export function TableauChargesGestionCourante({
  dossierId,
  initialData,
}: {
  dossierId: string;
  ...
  const setRowsStore = useCallback(
    (newRows: AutreChargeDateeRow[]) => store.setGestionCourante(dossierId, newRows),
    [dossierId, store],
  );
        ...
        >
          ...
          <Td className="w-20">
            <select
              ...
              value={row.tauxTVA}
              onChange={(e) => store.updateGestionCourante(dossierId, i, { tauxTVA: numVal(e.target.value) })}
            >
