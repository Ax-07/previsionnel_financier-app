...
const TYPE = "CHARGE_A_PAYER" as const;
...
export function TableauChargesAPayer({
  dossierId,
  initialData,
}: {
  dossierId: string;
  ...
  const handleSave = useCallback(() => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        store.markCAPSaved(dossierId, fresh);
  ...
  const setRowsStore = useCallback(
    (newRows: AutreChargeBilanRow[]) => store.setCAP(dossierId, newRows),
    [dossierId, store],
  );
        ...
        >
          ...
          <Td className="w-8 px-2">
            <input
              ...
              onChange={(e) => store.updateCAP(dossierId, i, { actif: e.target.checked })}
          ...
          <Td>
            <input
              className={cellInput}
              value={row.libelle}
              onChange={(e) => store.updateCAP(dossierId, i, { libelle: e.target.value })}
              placeholder="Libellé"
            />
          </Td>
          <Td>
            <select
              ...
              value={row.nature}
              onChange={(e) => store.updateCAP(dossierId, i, { nature: e.target.value })}
            >
              <option value="" className="bg-background text-foreground">—</option>
              {NATURES_CHARGE_BILAN_CAP.map((n) => (
          ...
          <Td className="w-28">
            <input
              ...
              value={row.montantN}
              min={0}
              onChange={(e) => store.updateCAP(dossierId, i, { montantN: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN1}
              min={0}
              onChange={(e) => store.updateCAP(dossierId, i, { montantN1: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN2}
              min={0}
              onChange={(e) => store.updateCAP(dossierId, i, { montantN2: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-10 px-1">
            <div className="flex items-center justify-center gap-0.5 opacity-0 group-hover:opacity-100">
              <Button size="icon" variant="ghost" className="h-6 w-6 text-primary" onClick={() => store.duplicateCAP(dossierId, i)}>
                <Copy className="h-3 w-3" />
              </Button>
              <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => store.removeCAP(dossierId, i)}>
