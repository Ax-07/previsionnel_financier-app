...
import { fetchChargesDatees, saveChargesDatees } from "@/app/actions/autres-charges";
...
const CATEGORIE = "EXCEPTIONNELLE" as const;
...
export function TableauChargesExceptionnelles({
  dossierId,
  initialData,
}: {
  dossierId: string;
  ...
  const rows = draft.exceptionnelles;
  const isDirty = draft.hasUnsavedExceptionnelles;
  const [isPending, startTransition] = useTransition();
  const invalidateControleStores = useReloadScenarioData();

  useEffect(() => {
    if (rows.length === 0 && initialData.length > 0) {
      store.setExceptionnelles(dossierId, initialData);
    }
  ...
  const handleSave = useCallback(() => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        store.markExceptionnellesSaved(dossierId, fresh);
  ...
  const setRowsStore = useCallback(
    (newRows: AutreChargeDateeRow[]) => store.setExceptionnelles(dossierId, newRows),
    [dossierId, store],
  );

  const setRowsDnd = useCallback(
    (updater: (prev: AutreChargeDateeRow[]) => AutreChargeDateeRow[]) => setRowsStore(updater(rows)),
    [rows, setRowsStore],
  );
        ...
        >
          ...
          <Td className="w-8 px-2">
            <input
              ...
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { actif: e.target.checked })}
          ...
          <Td>
            <input
              className={cellInput}
              value={row.libelle}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { libelle: e.target.value })}
              placeholder="Libellé"
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN ?? ""}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { dateN: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN}
              min={0}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { montantN: numVal(e.target.value) })}
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN1 ?? ""}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { dateN1: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN1}
              min={0}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { montantN1: numVal(e.target.value) })}
            />
          </Td>
          <Td>
            <input
              ...
              className={cellInput}
              value={row.dateN2 ?? ""}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { dateN2: e.target.value })}
            />
          </Td>
          <Td className="w-28">
            <input
              ...
              value={row.montantN2}
              min={0}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { montantN2: numVal(e.target.value) })}
            />
          </Td>
          <Td className="w-20">
            <select
              ...
              value={row.tauxTVA}
              onChange={(e) => store.updateExceptionnelle(dossierId, i, { tauxTVA: numVal(e.target.value) })}
            >
          ...
          <Td className="w-36">
            <select
              ...
              value={row.typeTVA ?? ""}
              onChange={(e) =>
                store.updateExceptionnelle(dossierId, i, {
          ...
          <Td className="w-10 px-1">
            <div className="flex items-center justify-center gap-0.5 opacity-0 group-hover:opacity-100">
              <Button size="icon" variant="ghost" className="h-6 w-6 text-primary" onClick={() => store.duplicateExceptionnelle(dossierId, i)}>
                <Copy className="h-3 w-3" />
              </Button>
              <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => store.removeExceptionnelle(dossierId, i)}>
  ...
  return (
    <div className="space-y-3">
      <SectionHeader
        ...
        isDirty={isDirty}
        isSaving={isPending}
        onAdd={() => store.addExceptionnelle(dossierId)}
        onSave={handleSave}
